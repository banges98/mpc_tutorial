...
class LocationSensor:
    def __init__(self, zone=52):
        ...
        # Set origin coordinates (Morai simulation ver.)
        self.e_o = 302459.942
    ...
    def navsat_callback(self, gps_msg):
        ...
        if dt > 0:
            ...
            x, y = e_global - self.e_o, n_global - self.n_o
            print(x, y)
            ...
            self.update_velocity(dis, dt)

            # Publish Odometry and Broadcast Transform
            self.publish_odom(x, y, heading)
            self.broadcast_tf(x, y, heading)
    ...
    def update_velocity(self, dis, dt):
        current_vel = dis / dt
        self.vel_buffer.append(current_vel)
        
        if len(self.vel_buffer) > self.buffer_size:
            self.vel_buffer.pop(0)
        
        self.vel = np.mean(self.vel_buffer)


    def publish_odom(self, x, y, heading):
    ...
    def broadcast_tf(self, x, y, heading):
        q = tf.transformations.quaternion_from_euler(0, 0, heading)
        
        br = tf.TransformBroadcaster()
        br.sendTransform((x, y, 0), q, rospy.Time.now(), "base_link", "map")


if __name__ == "__main__":
    rospy.init_node("gps_parser", anonymous=True)
    gps_odom = LocationSensor()
